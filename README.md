...
| 🤖 **AI 增强（可选，推荐）** | 可接入 AI API 辅助解析和评分                                                                 |
...
> 纯前端实现，无需任何构建步骤或服务器。也可直接双击 `index.html` 运行。
...
```
综合分 = max(OpenAlex, Crossref)   （赢者通吃）
```

| 字段 | 默认权重 | 算法                               |
| :--- | :------: | :--------------------------------- |
| 标题 |   48%    | Jaccard 词集相似度                 |
| 作者 |   20%    | 结构化姓名匹配 + Jaccard 补充      |
| 期刊 |   12%    | 编辑距离 + 缩写前缀匹配            |
| 年份 |   10%    | 精确匹配（±1 年给 50% 分）         |
| 页码 |    4%    | 起止页精确匹配（支持缩写页码展开） |
| 卷号 |    4%    | 精确匹配                           |
| 期号 |    2%    | 精确匹配                           |

> **et al. 处理**：检测到显式截断标记时，仅以列出的作者数计分，施加 15% 轻微罚分。

> **权重调整**：可通过「打分标准」按钮开启/关闭参与评分的字段，权重将自动重分配并更新评分。

![权重设置](assets/images/weights.png)
...
- ⚙️ 点击「打分标准」按钮可自定义评分权重（开启/关闭各字段）
- 🤖 推荐使用 AI 整理引用条目格式，尤其适用于从 PDF 复制时带有异常换行的引用文本
